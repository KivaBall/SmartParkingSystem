...
constexpr unsigned long LCD_RENDER_INTERVAL_MS = 250UL;
...
unsigned long lastLcdRenderedAt = 0;
...
void updateLcd()
{
    ...
    if (now - lastLcdRenderedAt < LCD_RENDER_INTERVAL_MS)
    {
        return;
    }

    lastLcdRenderedAt = now;
    lcd.setCursor(0, 0);
    lcd.print("                ");
    lcd.setCursor(0, 0);
    lcd.print(lcdLine1);
...
{
    beginProtocolFrame();
    COMM.print(F("PROFILE|board=ArduinoMega|rfid=MFRC522|lcd=I2C_16X2|gate=SERVO|transport=SERIAL|slots=6|route_led_strips=1|route_leds=15|front_sensor=1"));
    endProtocolFrame();
}
