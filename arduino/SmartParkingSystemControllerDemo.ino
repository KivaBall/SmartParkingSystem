...
// Bluetooth
// -----------------------------------------------------------------------------
// Demo-контролер працює через SoftwareSerial:
// D2 = RX
// D3 = TX
// HC-06 TXD -> D2
// HC-06 RXD <- D3 через дільник
...
void sendHello()
{
    if (DEMO_MODE)
    {
        btSerial.println(F("HELLO_OK|device=SMART_PARKING|fw=2|slots=6|transport=HC06"));
    }
    else
    {
...
void sendProfile()
{
    if (DEMO_MODE)
    {
        btSerial.println(F("PROFILE|board=ATmega328P|rfid=DEMO|lcd=DISABLED|gate=DEMO|transport=HC06|slots=6"));
    }
    else
    {
...
void triggerDemoCardEvent()
{
    if (config.forceGateLock)
    {
        ...
        btSerial.println(F("LOCKED|uid=B041CE32"));
        return;
    }
    ...
    btSerial.println(F("ALLOWED|uid=B041CE32"));
}
